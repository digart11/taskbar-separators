...
#error "Unsupported architecture"
    ...
    bool TryGetButtonBoundaryDividerGeometry(
        Controls::Canvas const &overlayCanvas,
        ...
        DividerGeometry *geometry)
    {
        ...
        if (!TryGetElementBounds(overlayCanvas, targetButton, &targetBounds) ||
            (previousButton &&
             !TryGetElementBounds(overlayCanvas, previousButton,
                                  &previousBounds)) ||
            (nextButton &&
        ...
        if (beforeFirst)
        {
            ...
            center = leadingEdge -
                     std::copysign(std::max(0.0, availableSpacing) / 2.0,
                                   direction);
        }
        else if (nextButton)
        {
        ...
        try
        {
            if (repeater && repeater.Dispatcher().HasThreadAccess())
            {
                ...
                DividerGeometryMode geometryMode =
                    orientationValid
                        ? DetectDividerGeometryMode(appButtons, appIcons,
                                                   taskbarOrientation)
                        : DividerGeometryMode::iconCenters;
                double primaryOrderingDirection = 0;
                bool primaryOrderingValid =
                    orientationValid &&
                    TryGetPrimaryOrderingDirection(
                        overlayCanvas, appButtons, taskbarOrientation,
                     ...
                     activeIndex++)
                {
                        ...
                        geometryMode == DividerGeometryMode::buttonBoundaries)
                    {
                        geometryValid = TryGetButtonBoundaryDividerGeometry(
                            overlayCanvas, previousButton, targetButton, nextButton,
                            activeSeparator.beforeFirst, false,
                            taskbarOrientation, snapshot.rootWidth,
                            snapshot.rootHeight, hostWidth, hostHeight, &geometry);
                    }
