        ...
        if (auto tooltipText = tooltipObject.try_as<winrt::hstring>())
{
    ...
    auto appendSeparator = [&](size_t settingsIndex, PCWSTR positionStr)
    {
        ...
        {
            separator.before = 1;
            beforeAndAfter = 1;   // Places separator both before and after
            ptr += 2;             // Skip "+-"
        }
        else if (ptr[0] == L'+')
        {
        ...
        else if (ptr[0] == L'-')      // Ignore "-"
        {
            separator.before = 0;
            ptr += 1;                 // Skip "-"
        }
        
        // If the string was only "+" or "+-" or "-", ignore it
        if (!*ptr)
        {
            return;
        }

        // Parse the remaining string (e.g. "5" or "File Explorer")
        wchar_t *end;
        ...
        {
            separator.appName = ptr;  // Stores stripped string (e.g. "File Explorer")
        }

        // Add first separator (before or after)
        settings.separators.push_back(separator);
        if (beforeAndAfter)
        {
            // Add a second separator for after
            separator.before = 0;
        ...
        try
        {
            if (repeater && repeater.Dispatcher().HasThreadAccess())
            {
                ...
                for (auto &separator : settings.separators)
                {
                    ...
                    if (position <=0 || std::find(usedPositions.begin(), usedPositions.end(),
